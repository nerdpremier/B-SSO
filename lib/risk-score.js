...
export async function ensureBehaviorRisksSchema() {
    ...
    // SECURITY FIX: Add risk_level column to oauth_tokens for tracking token risk
    try {
        await pool.query(
            `ALTER TABLE oauth_tokens ADD COLUMN IF NOT EXISTS risk_level TEXT DEFAULT 'LOW'`
        );
    } catch { /* ignore */ }

    // SECURITY FIX: Add step_up_required column to oauth_tokens for MFA tracking
    try {
        await pool.query(
            `ALTER TABLE oauth_tokens ADD COLUMN IF NOT EXISTS step_up_required BOOLEAN DEFAULT FALSE`
        );
    } catch { /* ignore */ }
}
